...
router.post("/", async (req, res) => {
    ...
    if (!email) return res.status(401).json({ status: "error", data: null, message: "Email not provided" });
    ...
    const {forgotPasswordToken} = generateJwtToken(payload as any, "forgotPassword") 
