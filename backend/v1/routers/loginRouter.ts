...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        if (user.email == undefined || user.userId == undefined || user.passwordHash == undefined) {
        ...
        bcrypt.compare(password, user.passwordHash, async function(err, result) {
            ...
            if (result){
                ...
                const storeTokenRes = await Tokens.addRefreshToken( refreshToken as string, user.userId as string)
