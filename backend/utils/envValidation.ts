...
const envVars: EnvVarConfig[] = [
  ...
  // JWT Secrets
  { name: 'ACCESS_TOKEN_SECRET', required: true, description: 'Access token secret' },
  { name: 'REFRESH_TOKEN_SECRET', required: true, description: 'Refresh token secret' },
  { name: 'FORGOT_PASSWORD_TOKEN_SECRET', required: true, description: 'Forgot password token secret' },
  { name: 'TOKEN_EXPIRY', required: true, description: 'Token expiry time' },
  { name: 'TOKEN_EXPIRY_ABSOLUTE', required: true, description: 'Token absolute expiry time' },
  { name: 'TOKEN_EXPIRY_SECONDS', required: true, description: 'Token expiry in seconds' },
  ...
  { name: 'SENDGRID_PASSWORD', required: true, description: 'SendGrid password' },
  ...
  // CORS
  { name: 'CORS_ALLOWED_ORIGINS', required: false, description: 'Comma-separated list of allowed CORS origins' },
  
  // Security
  { name: 'CLOUDINARY_DOMAIN', required: false, description: 'Cloudinary domain' },
];
...
export function validateEnv(): void {
  ...
  if (warnings.length > 0) {
    logger.warn({ warnings }, 'Optional environment variables not set (using defaults)');
  }
